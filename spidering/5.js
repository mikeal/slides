...
process.on('exit', function () {
  console.error('number,dish,restaurant,address,telephone,info,title')
...
s.route('blogs.sfweekly.com', '/foodie/sfoodies_92/index.php*', function (window, $) {
  $('div.primaryCategory:contains("SFoodie\'s 92")').each(function () {
    var entry = $(this).parent()
      , title = entry.find('h2').text()
      ;

    number = parseInt(title.slice(title.indexOf(' ')+1, title.indexOf(':')))
